...
#   use_sim_time = LaunchConfiguration('use_sim_time', default='true')
   #!!!!****Want to change out for electrical room start point to see if it will localize!!!!!!****
...
#       'map': os.path.join(nav2_file_dir, 'map', 'map.yaml'),
 	    'map': os.path.join(os.path.expanduser('~'), 'sweepbot_ws', 'map', 'EERCpentElec.yaml'),
...
#********"create_bringup create_2.launch" does not have the ending .py???
    ...
    map_server_cmd = Node(
        ...
        output='screen',
        parameters=[{
            'yaml_filename': os.path.join(os.path.expanduser('~'), 'sweepbot_ws', 'map', 'EERCpentElec.yaml')
        }]
    )
