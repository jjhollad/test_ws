...
DEFAULT_MAP_NAME = 'FullSBMap'
DEFAULT_MAP_PATH = '/home/user/maps/EERCsB/FullSBMap.yaml'
...
def generate_launch_description():
    ...
    publish_initial_pose_arg = DeclareLaunchArgument(
        ...
        default_value='false',
        description='Publish the configured start pose on /initialpose; leave false when using RViz 2D Pose Estimate',
    )
    coverage_csv_path_arg = DeclareLaunchArgument(
        'coverage_csv_path',
        default_value='/home/user/test_ws/opencv_fullsb_main_square_preview/center_loop_path.csv',
        description='CSV path generated by opencv_wall_trace_demo.py; required by default',
    )
    require_coverage_csv_arg = DeclareLaunchArgument(
        'require_coverage_csv',
        default_value='true',
        description='Require OpenCV-generated coverage CSV; set false to allow legacy /map fallback',
    )
    graceful_navigation_arg = DeclareLaunchArgument(
        'graceful_navigation',
        default_value='true',
        description='Send short adaptive NavigateToPose goals instead of one large FollowWaypoints goal',
    )
    live_costmap_topic_arg = DeclareLaunchArgument(
        'live_costmap_topic',
        default_value='/global_costmap/costmap',
        description='OccupancyGrid/costmap topic used to skip currently blocked coverage targets',
    )
    goal_stride_arg = DeclareLaunchArgument(
        'goal_stride',
        default_value='6',
        description='Use every Nth suggested coverage pose as the next Nav2 target; larger values reduce path recomputation',
    )
    goal_clearance_arg = DeclareLaunchArgument(
        'goal_clearance',
        default_value='0.15',
        description='Meters of live-costmap clearance required before sending an adaptive target',
    )
    revisit_deferred_goals_arg = DeclareLaunchArgument(
        'revisit_deferred_goals',
        default_value='true',
        description='Try blocked/skipped adaptive targets one more time after the first pass',
    )
    ...
    coverage_cleaner = Node(
        ...
        parameters=[{
            ...
            'coverage_csv_path': LaunchConfiguration('coverage_csv_path'),
            'require_coverage_csv': ParameterValue(
                LaunchConfiguration('require_coverage_csv'),
                value_type=bool,
            ),
            'graceful_navigation': ParameterValue(
                LaunchConfiguration('graceful_navigation'),
                value_type=bool,
            ),
            'live_costmap_topic': LaunchConfiguration('live_costmap_topic'),
            'goal_stride': ParameterValue(LaunchConfiguration('goal_stride'), value_type=int),
            'goal_clearance': ParameterValue(LaunchConfiguration('goal_clearance'), value_type=float),
            'revisit_deferred_goals': ParameterValue(
                LaunchConfiguration('revisit_deferred_goals'),
                value_type=bool,
            ),
        }],
    )
    ...
    return LaunchDescription([
        ...
        coverage_csv_path_arg,
        graceful_navigation_arg,
        live_costmap_topic_arg,
        goal_stride_arg,
        goal_clearance_arg,
        revisit_deferred_goals_arg,
        line_spacing_arg,
